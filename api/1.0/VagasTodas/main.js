document.addEventListener("DOMContentLoaded", function () {
    ...
    var pagina = document.getElementById("paginaAtual").value; // pegando o valor atual da pagina
    loadTableData("asc", pagina); // Carregar os dados em ordem crescente por padrão
});

function loadTableData(order, page) {
    var divJobs = document.getElementById("all-jobs");
    ...
    xhr.open("GET", "api/1.0/VagasTodas/index.php?order=" + order + "&pagina=" + page, true);
    xhr.onreadystatechange = function () {
        if (xhr.readyState === 4 && xhr.status === 200) {
            var dados = JSON.parse(xhr.responseText);

            // Selecione o elemento onde você deseja adicionar as divs (por exemplo, o body)
            var container = document.getElementById("all-jobs");
